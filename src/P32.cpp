...
double P32::simulate(Arguments& paras)
{
	...
	double S_T_sum = 0.0;
	size_t iDays = get_dt() * 365;
	bool up_reach = false;
	bool down_reach = false;
	for (size_t i = 1; i <= iDays; i++)
	{
		double ST_i = simulate_time(i/365.0);
		S_T_sum += ST_i;
		if (paras.has("UP") && ST_i >= paras.g_VAL<double>("UP"))
		{
			up_reach = true;
		}

		if (paras.has("DOWN") && ST_i <= paras.g_VAL<double>("DOWN"))
		{
			down_reach = true;
		}
	}

	paras.g_SET<double>("ST_AVER", new double(S_T_sum / iDays));

	if (paras.has("UP"))
	{
		paras.g_SET<bool>("UP_REACH", new bool(up_reach));
	}

	if (paras.has("DOWN"))
	{
		paras.g_SET<bool>("UP_DOWN", new bool(down_reach));
	}
    return 0.0;
}
