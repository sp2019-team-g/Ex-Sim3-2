...
#include <iostream>
#include <fstream>
McPricingAS::McPricingAS(Option*opt, Process*pro) : PricingEng(opt){
    pro_ = pro;
}

McPricingAS::McPricingAS(Arguments& paras) : PricingEng(paras)
{
    pro_ = paras.g_PTR<Process>("process");
}

double McPricingAS::price(){throw BadAccess_Exception();}

double McPricingAS::price(Arguments& paras)
{
    ...
    for(size_t i = 0; i < bsize; i++)
    {
        ...
        Path * path = pro_->simulatePath(paras);
